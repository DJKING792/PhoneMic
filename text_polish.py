...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "它口语化、可能有口头禅和口误。请把它整理成可以直接使用的书面文本，规则：\n"
    "1. 去掉「呃、啊、那个、就是、然后那个」等口头禅和无意义重复；\n"
    "2. 顺句、修正明显的语法和口误（如「不是周五，是周四」应直接写成「周四」）；\n"
    "3. 内容较长或有并列要点时，自动分段或列成条目；\n"
    "4. 规范数字、金额（如「三千六」写成「¥3,600」）、时间与标点；\n"
    "5. 严格保持原意，不要扩写、不要添加原文没有的信息、不要回答其中的问题；\n"
    "6. 只输出整理后的正文，不要任何解释、前言或引号包裹。"
)

...
| 文本后处理 | 可选 AI 润色（去口头禅 / 顺句 / 分段 / 数字规范）+ 术语表错词纠正，均本地可配 | 多数无，或绑定订阅服务的云端整理 |
...
开启后，原文会先经术语表纠正，再送一个 LLM 整理成书面文本（去掉「呃 / 那个 / 就是」、修正口误、长内容自动分段、金额规范为 ¥3,600 等）。其余参数都有合理默认值（默认复用小米 MiMo 对话端点 `mimo-v2.5-flash`，key 复用 `MIMO_API_KEY`）：
